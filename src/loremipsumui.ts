...
export default class LoremIpsumUI extends Plugin {
  init() {
    ...
    const LOREM_TEXT_TYPE = { PARAGRAPH: 1, SENTENCE: 2, WORD: 3 };
    const LOREM_WORDS = [
     ...
     */
    function generateLoremIpsum(count, type) {
    ...
    editor.ui.componentFactory.add('loremIpsum', locale => {
      ...
      view.on('execute', () => {
        editor.model.change(writer => {
          editor.model.insertContent(writer.createHtml(generateLoremIpsum(1, LOREM_TEXT_TYPE.PARAGRAPH)));
        });
      });
